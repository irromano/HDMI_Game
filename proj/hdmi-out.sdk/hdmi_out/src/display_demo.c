...
//Added libraries for btn and timer interupts
#include "xgpio.h"
#include "xtmrctr.h"
#include "xscugic.h"
...
//Function made to move a player
void move(int move);
...
void DemoInitialize()
{
	...
	//Initialize the btn interrupt
	Status = XGpio_Initialize(&BTNInst, BTNS_DEVICE_ID);
	if(Status != XST_SUCCESS) {
		xil_printf("Couldn't initialization the button interrupt %d\r\n", Status);
		return;
	}
	...
	if(Status != XST_SUCCESS) {
		xil_printf("Couldn't initialization the timer interrupt %d\r\n", Status);
		return;
	}
...
void DemoPrintTest(u8 *frame, u32 width, u32 height, u32 stride, int pattern)
{
	u32 xcoi, ycoi;
	u32 iPixelAddr;
	u8 wRed, wBlue, wGreen, pRed, pBlue, pGreen;
	u32 wCurrentInt;
	...
	case DEMO_PATTERN_2:
		...
		for(xcoi = 0; xcoi < (width * 4); xcoi+=4)
		{
			...
			//This is a purple color for player 1
			pRed = 64;
			pBlue = 0;
			pGreen = 64;
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				if (xcoi > pbox->xpos && xcoi < pbox->xpos + pbox->size && ycoi > pbox->ypos && ycoi < pbox->ypos + pbox->size/4) {
					frame[iPixelAddr] = pRed;
					frame[iPixelAddr + 1] = pBlue;
					frame[iPixelAddr + 2] = pGreen;
				} else {
					frame[iPixelAddr] = wRed;
					frame[iPixelAddr + 1] = wBlue;
					frame[iPixelAddr + 2] = wGreen;
...
void BTN_Intr_Handler(void *InstancePtr)
{
	...
	//This moves a player based on the button pressed
	move(btn_value);
